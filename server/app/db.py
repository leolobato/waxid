...
class Database:
    ...
    def get_tracks_for_album(self, album_id: int) -> list[dict]:
        rows = self.conn.execute(
            ...
            "WHERE t.album_id = ? "
            "GROUP BY t.track_id ORDER BY t.side, t.track_number, t.position, t.track_id",
            (album_id,),
        ).fetchall()
        return [dict(r) for r in rows]
    ...
    def get_tracks(self) -> list[dict]:
        rows = self.conn.execute(
            "SELECT t.*, COUNT(h.hash) as num_hashes "
            "FROM tracks t LEFT JOIN hashes h ON t.track_id = h.track_id "
            "GROUP BY t.track_id ORDER BY t.album_id, t.side, t.track_number, t.position, t.track_id"
        ).fetchall()
        return [dict(r) for r in rows]
